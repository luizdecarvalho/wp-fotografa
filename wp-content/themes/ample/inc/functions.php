   ...
   if ( 'post' == get_post_type() ) : ?>
      <div class="entry-meta clearfix">
         ...
         <?php if ( comments_open() ) { ?>
            <span class="comments"><i class="fa fa-aw fa-comment"></i><?php comments_popup_link( __( 'Sem Comentários', 'ample' ), __( '1 Comentário', 'ample' ), __( '% Comments', 'ample' ), '', __( 'Comments Off', 'ample' ) ); ?></span>
         <?php } ?>
      </div>
   <?php endif;
}
   ...
   <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
      <article id="comment-<?php comment_ID(); ?>" class="comment">
         <header class="comment-meta comment-author vcard">
            <?php
               ...
               printf( '<div class="comment-author-link"><i class="fa fa-user"></i>%1$s%2$s</div>',
                  ...
                  ( $comment->user_id === $post->post_author ) ? '<span>' . __( 'Post da Fotógrafa', 'ample' ) . '</span>' : ''
               );
               printf( '<div class="comment-date-time"><i class="fa fa-calendar-o"></i>%1$s</div>',
                  sprintf( __( '%1$s às %2$s', 'ample' ), get_comment_date(), get_comment_time() )
               );
         ...
         <?php if ( '0' == $comment->comment_approved ) : ?>
            <p class="comment-awaiting-moderation"><?php _e( 'Seu comentário será analisado.', 'ample' ); ?></p>
         <?php endif; ?>

         <section class="comment-content comment">
            <?php comment_text(); ?>
            <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Responder', 'ample' ), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
         </section>
